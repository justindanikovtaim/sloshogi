...
function decideBestMove(){
    ...
    for(xray=0;xray<firstMovesArray.length;xray++){
        ...
        gameState[firstMovesArray[xray][0]] = gameState[firstMovesArray[xray][1]];
    ...
    for(xyz=0; xyz<moveAnalyzeArray.length; xyz++){
        if(moveAnalyzeArray[xyz] < 1){
            ...
            break;
        } else if(moveAnalyzeArray[xyz] < fewestMoves){ //if the move has fewer possiblites than the one currently set
            fewestMoves = moveAnalyzeArray[xyz];//set it to the new best move found so far
        }
    }
    if(bestMove.length <3){//if the best move hasn't been set yet (meaning there are no moves that would result in no checkable responses)
...
function countCheckingMoves(){
    ...
    for(n=0;n<80;n++){
        ...
        if(gameState[n].charAt(0) == "B"){
            ...
            for(z=0;z<length;z++){ 
                let tryTheMove = tryMove(gameState[n], n, tempCounterArray[z], gameState[n]);
                if( tryTheMove == "check"){
            ...
            for(z=length; z<tempCounterArray.length;z++){
                let tryTheMove = tryMove("BN"+gameState[n].substr(1), n, tempCounterArray[z], gameState[n]);
                if( tryTheMove == "check"){
                    numberOfCheckingMoves ++;
                }else if(tryTheMove == "mate"){
                    numberOfCheckingMoves += 1000; //add 1,000 for a checkmate so that we know not to move there
    ...
    for(bboy=8;bboy<14;bboy++){//go through mochigoma
        if (mochiGomaArray[bboy] > 0) {//if there is actually a mochigoma in that spot
            turn++;
            ...
            let surroundingSquares = [whiteGyokuPos-1, whiteGyokuPos+1, whiteGyokuPos-10,whiteGyokuPos-8,whiteGyokuPos-9,whiteGyokuPos+8,whiteGyokuPos+9,whiteGyokuPos+10]
            ...
            for(y=0;y<newMoves.length;y++){
                let tryTheMove = tryMove(mochiGomaOrder[bboy].substr(1), 81, newMoves[y],mochiGomaOrder[bboy].substr(1));
            if( tryTheMove == "check"){
                numberOfCheckingMoves ++;
            }else if(tryTheMove == "mate"){
                numberOfCheckingMoves += 1000; //add 1,000 for a checkmate so that we know not to move there
