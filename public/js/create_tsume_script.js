...
for (jupiter = 0; jupiter < 2; jupiter++) {
    ...
    for (x = 0; x < 7; x++) {
        if (jupiter === 0) {
            ...
            mochiGomaAmmount[x].setAttribute(
                "id",
                mochiGomaOrder[x] + "Counter"
            );
            document
            ...
            mochiGoma[x + 7].setAttribute(
                "onClick",
                "mochiGomaIncrement(this.id)"
            );
            ...
            mochiGomaAmmount[x + 7].setAttribute(
                "id",
                mochiGomaOrder[x + 7] + "Counter"
            );
            document
                .getElementById("blackMochigoma")
...
for (jupiter = 0; jupiter < 4; jupiter++) {
    for (x = 0; x < 8; x++) {
        if (jupiter === 0) {
            //if it's the first time through, we are drawing the white pieces
            selectionGoma[x] = document.createElement("img"); //create a new img element for each mochigoma type
            selectionGoma[x].src =
                "/public/images/koma/1/" + selectionGomaOrder[x] + ".png";
            ...
            document
                .getElementById("choosePieceId")
                .appendChild(selectionGoma[x]);
        } else if (jupiter === 1) {
            //drawing the white narigoma
            selectionGoma[x] = document.createElement("img"); //create a new img element for each mochigoma type
            selectionGoma[x].src =
                "/public/images/koma/1/" + selectionGomaOrder[x + 8] + ".png";
            ...
            //drawing the black narigoma
            selectionGoma[x] = document.createElement("img"); //create a new img element for each mochigoma type
            selectionGoma[x].src =
                "/public/images/koma/1/" + selectionGomaOrder[x + 16] + ".png";
            ...
            //otherwise it's the last time through, so we are drawing the black pieces
            selectionGoma[x + 9] = document.createElement("img"); //create a new img element for each mochigoma type
            selectionGoma[x + 9].src =
                "/public/images/koma/1/" + selectionGomaOrder[x + 24] + ".png";
...
function pieceClick(id) {
    ...
    if (id < 44) {
        document
            .getElementById("choosePieceId")
            .classList.toggle("pieceSelectionShowLow");
    } else if (id > 44) {
        document
            .getElementById("choosePieceId")
            .classList.toggle("pieceSelectionShowHigh");
    }
    selectedSquare = id;
}
function mochiGomaIncrement(id) {
    ...
    if (ammount > 1) {
        ...
        document.getElementById(id + "Counter").src =
            "/public/images/mochiGomaNum" + ammount + ".png";
    } else {
...
function drawBoard() {
    ...
    for (x = 0; x < boardConfiguration.length; x++) {
        boardSquare[x].src =
            "/public/images/koma/1/" + boardConfiguration[x] + ".png";
    }

    //draw mochigoma
    for (i = 0; i < 8; i += 7) {
        for (x = 0; x < 7; x++) {
            if (mochiGomaConfiguration[x + i] > 0) {
...
function toCreatePage() {
    document.getElementById("boardConfig").value =
        boardConfiguration.toString();
    document.getElementById("mochigomaConfig").value =
        mochiGomaConfiguration.toString();
