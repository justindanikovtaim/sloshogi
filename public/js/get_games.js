...
for (i = 0; i < currentGameIdArray.length; i++) {
    ...
    if (currentGameOpponentArray[a + 1] === 1) {
        ...
        currentGameLink[i].innerHTML =
            "SLO" +
            currentGameIdArray[i] +
            " | " +
            currentGameOpponentArray[a] +
            " to play";
    } else {
        currentGameLink[i].innerHTML =
            "SLO" +
...
for (i = 0; i < newChallengesArray.length; i++) {
    ...
    challengesGameLink[i].href = "/view-challenge?id=" + newChallengesArray[i];
