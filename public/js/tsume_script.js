...
let playerMoveSequence = mainSequence; //playerMoveSequence will be modified with each move, while mainSequence will be the base to reset to

setMessage(problemName);
...
for (i = 0; i < 9; i++) {
    for (x = 0; x < 9; x++) {
        ...
        boardSquare[sC].setAttribute(
            'onclick',
            'pieceClick(Number(this.id), false)',
        ); //run the piececlick funtion when clicked
...
function movePiece(id, piece) {
    ...
    if (piece < 81) {
        ...
        if (gameState[piece].charAt(1) !== 'N' && promoteZone) {
    ...
    if (checkForCheck('W') != 'W') {
        ...
        if (playerMoveSequence.length === 3) {
            ...
            playerMoveSequence[0] === piece &&
            playerMoveSequence[1] === id &&
            playerMoveSequence[2] === gameState[piece]
        ) {
            //if the move matches
...
function wrongMove(playNoPlay) {
    if (playNoPlay) {
        ...
        setTimeout(function () {
            ...
            if (wrongMoveResponse.length === 1) {
                //if there was an unforseen way to checkmate
                tsumeSolved();
            } else {
...
function tsumeSolved() {
    ...
    if (isGuest) {
        ...
        let json = JSON.stringify({
            scoreBoard: finalTime,
            problemId: currentGameID,
            startingTime: originalTimeLimit,
        });
...
function makeNewAccount(solvedTime) {
    document.getElementById('solvedTimeNewAccount').value = solvedTime;
